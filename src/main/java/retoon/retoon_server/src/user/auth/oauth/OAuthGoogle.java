package retoon.retoon_server.src.user.auth.oauth;
...
import retoon.retoon_server.src.user.auth.token.GoogleToken;
...
public class OAuthGoogle implements OAuthSocial {
    ...
    public String extractAccessToken(String code) throws JsonProcessingException {
        ...
        if(responseEntity.getStatusCode() == HttpStatus.OK){
            ...
            GoogleToken googleToken = mapper.readValue(responseEntity.getBody(), GoogleToken.class); //JSON -> Java Object, Token Request
            //응답 객체에서 id token 추출
            return googleToken.getId_token(); //구글 로그인에서 id token이 사용자 정보에 접근하는 access token
        }
    ...
    public SocialProfileDto getUserProfileInfo(String accessToken) {
        ...
        OAuthGoogleUser oAuthGoogleUser = restTemplate.getForObject(requestUrl, OAuthGoogleUser.class);
        System.out.println(">> google user information :: {}" + oAuthGoogleUser);

        //공통적으로 추출할 수 있는 정보에 대한 객체에 삽입
        return new SocialProfileDto(oAuthGoogleUser.getName(), oAuthGoogleUser.getEmail());
    }
}
