...
import retoon.retoon_server.login.token.KakaoTokenRes;
...
public class KakaoOauth implements SocialOauth {
    ...
    @Value("${logout.kakao.user.url}")
    private String KAKAO_LOGOUT_USER_URL;
    @Value("${logout.kakao.redirect.url}")
    ...
    public String requestAccessToken(String code) throws JsonProcessingException {
        ...
        if(responseEntity.getStatusCode() == HttpStatus.OK){
            ...
            KakaoTokenRes kakaoTokenRes = mapper.readValue(responseEntity.getBody(), KakaoTokenRes.class); //JSON -> Java Object, Token Request
            //응답 객체에서 real access token 추출
            return kakaoTokenRes.getAccess_token();
        }
    ...
    public GetSocialUserRes getUserInfo(String accessToken){
        ...
        //사용자 정보 추출
        System.out.println(">> kakao user information :: {}" + socialUserRes);
    ...
    public String logout(){
        Map<String, Object> params = new HashMap<>();
        params.put("client_id", KAKAO_LOGIN_CLIENT_ID);
        params.put("logout_redirect_uri", KAKAO_LOGOUT_REDIRECT_URL);
        ...
        return KAKAO_LOGOUT_USER_URL + "?" + parameterString;
    }
}
